...
# this template is based on F5 supported template f5-existing-stack-same-net-cluster-payg-2nic-bigip
# Copyright 2021 F5 Networks All rights reserved.
#
# Version 3.14.0
# this modified version allow to use a proxy for Internet communications 
...
## Proxy
  if str(context.properties['proxyAddr']).lower() != 'none':
      PROXYCFG = '--db proxy.host:' + context.properties['proxyAddr'] + ' --db proxy.port:' + str(context.properties['proxyPort']) + ' --db proxy.protocol:' + context.properties['proxyProtocol']
      CURLSTG = '--proxy ' + context.properties['proxyProtocol'].lower() + '://' + context.properties['proxyAddr'] + ':' + str(context.properties['proxyPort'])
  #    if str(context.properties['proxyUser']).lower() != 'none':
  #      PROXYCFG = PROXYCFG + ' --db proxy.username:' + context.properties['proxyUser'] + ' --db proxy.password:' + context.properties['proxyPasswd']
  #      CURLSTG = CURLSTG + ' -U ' + context.properties['proxyUser'] + ':' + context.properties['proxyPasswd']
  else:
    PROXYCFG = ''
    CURLSTG = ''
  ...
  metadata = {
              'items': [{
                  ...
                  'value': ('\n'.join(['#!/bin/bash',
                                    ...
                                    'fi',
                                    '/usr/bin/curl ' + CURLSTG + ' -s -f --retry 20 -o /config/cloud/f5-cloud-libs.tar.gz https://cdn.f5.com/product/cloudsolutions/f5-cloud-libs/v4.26.5/f5-cloud-libs.tar.gz',
                                    '/usr/bin/curl ' + CURLSTG + ' -s -f --retry 20 -o /config/cloud/f5-cloud-libs-gce.tar.gz https://cdn.f5.com/product/cloudsolutions/f5-cloud-libs-gce/v2.9.1/f5-cloud-libs-gce.tar.gz',
                                    '/usr/bin/curl ' + CURLSTG + ' -s -f --retry 20 -o /var/config/rest/downloads/f5-appsvcs-3.31.0-6.noarch.rpm https://cdn.f5.com/product/cloudsolutions/f5-appsvcs-extension/v3.31.0/f5-appsvcs-3.31.0-6.noarch.rpm',
                                    '/usr/bin/curl ' + CURLSTG + ' -s -f -L --retry 20 -o /var/config/rest/downloads/f5-cloud-failover-1.9.0-0.noarch.rpm https://github.com/F5Networks/f5-cloud-failover-extension/releases/download/v1.9.0/f5-cloud-failover-1.9.0-0.noarch.rpm',
                                    ...
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/@f5devcentral/f5-cloud-libs/scripts/onboard.js --db provision.managementeth:eth1 ' + PROXYCFG + ' --host localhost -o /var/log/cloud/google/mgmt-swap.log --log-level ' + str(context.properties['logLevel']) + ' --wait-for CREATE_USER_DONE --signal MGMT_SWAP_DONE >> /var/log/cloud/google/install.log < /dev/null &',
...
def GenerateConfig(context):
  ...
  FirewallRuleMgmt(context),
  {
    ...
    'properties': Instance(context, 'create', storageName, 'payg', '1', context.properties['availabilityZone1'], network1SharedVpc, instanceName0, instanceName0)
  },{
    'name': instanceName1,
    'type': 'compute.v1.instance',
    'properties': Instance(context, 'join', storageName, 'payg', '2', context.properties['availabilityZone2'], network1SharedVpc, instanceName1, instanceName0)
  },{
    'name': storageName,
